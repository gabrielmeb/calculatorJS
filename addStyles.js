...
const outputClass = 
    "flex flex-col w-3/4 h-12 justify-center items-end m-1 bg-gray-300 text-gray-900 font-semibold px-2 border border-black hover:border-gray-600 rounded";
const output = document.querySelector(".output");
outputClass.split(' ').forEach((cls) => output.classList.add(cls));

const previousNumberClass = 
    "previousNumber flex max-w-lg h-1/3 pb-5 pt-1 text-gray-600 text-xs";
const previousNumber = document.querySelector(".previousNumber");
previousNumberClass.split(' ').forEach((cls) => previousNumber.classList.add(cls));

const currentNumberClass = 
    "currentNumber flex max-w-lg h-2/3 pb-2 text-xl"
const currentNumber = document.querySelector(".currentNumber");
currentNumberClass.split(' ').forEach((cls) => previousNumber.classList.add(cls));
...
function addClasses(button){
    const btnNumberClass = "hover:bg-green-500 border-white-500";
    ...
    const baseClass =
        "flex w-12 h-12 justify-center items-center m-1 font-bold py-2 px-2 border hover:border-transparent rounded text-white border-2 text-2x1 hover:bg-green-500 border-white-500"
